/**
 * migrate-to-calendar-entries.ts
 *
 * Migrates all users from the legacy calendar data model to the new unified
 * `calendarEntries` path.
 *
 * OLD model:
 *   users/{uid}/calendar/{date}         = string[]       (activityId array)
 *   users/{uid}/activityNotes/{date}/{activityId} = string (free-text note)
 *
 * NEW model:
 *   users/{uid}/calendarEntries/{date}/{pushId} = CalendarEntry {
 *     id, type: 'activity', activityId, time: null, note?
 *   }
 *
 * NOTE: calendarNotes (day notes) are left unchanged.
 *
